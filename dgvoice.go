...
func SendPCM(v *discordgo.VoiceConnection, pcm <-chan []int16, channels int) {
	if pcm == nil {
		return
	}
...
// Must say if this is mono or stereo. Ignore all other channels because discord
// maxxes out at stereo
// end time is the point in the audio that it was stopped. this is to allow for resuming
// later on
func PlayAudioFile(v *discordgo.VoiceConnection, filename string, stop <-chan bool,
	endtime chan<- float64, skipahead float64, isMono bool) {
	...
	run := exec.Command("ffmpeg", "-i", filename,
		"-f", "s16le",
		"-ar", strconv.Itoa(frameRate),
		"-ac", strconv.Itoa(channels),
		"-ss", strconv.FormatFloat(skipahead, 'f', -1, 32),
		"pipe:1")
	...
	go func() {
		<-stop
		endtime <- (time.Since(starttime).Seconds() + skipahead)
